...
foreach ( $items as $item_id => $item ) :
	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		// Quantity
		echo "\n" . sprintf( esc_html__( 'Quantity: %s', 'creatividad' ), $item['qty'] );

		// Cost
		echo "\n" . sprintf( esc_html__( 'Cost: %s', 'creatividad' ), $order->get_formatted_line_subtotal( $item ) );
		...
		if ( $show_download_links && $_product->exists() && $_product->is_downloadable() ) {
			...
			foreach ( $download_files as $download_id => $file ) {
				...
				if ( count( $download_files ) > 1 ) {
					$prefix = sprintf( esc_html__( 'Download %d', 'creatividad' ), $i );
				} elseif ( $i == 1 ) {
					$prefix = esc_html__( 'Download', 'creatividad' );
				}
