 ...
 class WPMETA_MediaAccess
{
	...
	public function __construct(array $a = array())
	{
		...
		if ( ! defined('WPMETA_SEND_TO_EDITOR_ENABLED'))
		{
			add_action('admin_footer', array($this, 'init'));

			define('WPMETA_SEND_TO_EDITOR_ENABLED', true);
		}
	}
		...
		if ($uri AND in_array($file, array('post.php', 'post-new.php')))
		{
			...
			/* <![CDATA[ */
				...
				jQuery(function($)
				{
					...
					{
						var WPMETA_insert_button_label = '';

						var WPMETA_mediafield = null;

						var WPMETA_send_to_editor_default = send_to_editor;

						send_to_editor = function(html)
						{
							clearInterval(interval);

							if (WPMETA_mediafield)
							{
								...
								WPMETA_mediafield.val(url);

								// reset insert button label
								setInsertButtonLabel(WPMETA_insert_button_label);

								WPMETA_mediafield = null;
							}
							else
							{
								WPMETA_send_to_editor_default(html);
							}
						...
						$('[class*="<?php echo $this->button_class_name; ?>"]').on('click', function()
						{
							...
							WPMETA_mediafield = $('.<?php echo $this->field_class_name; ?>-' + name, $(this).closest('.postbox'));
							...
							function iframeSetup()
							{
								...
								{
									// run once
									if ( ! WPMETA_insert_button_label.length)
									{
										WPMETA_insert_button_label = getInsertButtonLabel();
									}
